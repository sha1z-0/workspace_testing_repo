...
>(
  (
    ...
    // Close sidebar by default on mobile screens when component mounts
    React.useEffect(() => {
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-8 w-8 md:h-7 md:w-7 flex items-center justify-center", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
const sidebarMenuItemVariants = cva(
  "group relative flex flex-1 cursor-pointer items-center justify-between rounded-md px-3 py-2 text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
  {
    variants: {
      variant: {
        default: "",
